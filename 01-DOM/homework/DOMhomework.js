...
nombre.innerHTML = nombre.innerHTML + " Fernando S"
...
function buildToDos(toDos) {
  ...
  const array = toDos.map((todo, index) => buildToDo(todo, index))
...
function displayToDos() {
  ...
  let resultadoFuncion = buildToDos(toDoItems)

  resultadoFuncion.map((iteracion) => toDoContainer.appendChild(iteracion))
}
...
function addToDo() {
  ...
  let valorIngresado = document.getElementById("toDoInput").value
  let nuevoValor = new ToDo(valorIngresado)

  toDoItems.push(nuevoValor)

  let valor = document.getElementById("toDoInput")
  valor.value = ("")
